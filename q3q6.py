...
def simulate(input_strength, inhi_input, exci_input, T, dt):
    ...
    for t in np.arange(0, T, dt):
        RI = input_values[int(t / dt)]
        iaf_neuron.step(RI, dt)

    return len(iaf_neuron.getSpikes()) / (T / 1000)

def worker(p_i, input_neuron_strength, input_exci_neuron_num_range, input_inhi_neuron_num_range, input_neuron_freq, T, dt, shared_i, lock, frequencies_list):

    print("Calculating poisson procces input neurons spikes for " + str(max(input_exci_neuron_num_range)) + " neurons")
    ...
    for n in input_exci_neuron_num_range:
        ...
        with lock:
            shared_i.value += 1
            print(f"    {shared_i.value}/{max(input_exci_neuron_num_range)}", end='\r')
...
def fullsim1(T, dt, input_neuron_freq):
    ...
    # Parameters
    input_exci_neuron_num_range = np.arange(20, 5100, 20) # nnumber of excitatory neurons simulated by a poisson process
    input_inhi_neuron_num_range = np.arange(20, 5100, 20) # nnumber of inhibitory neurons simulated by a poisson process
    input_neuron_strength_range = np.arange(0.1, 7.1, 1.0) # mV
    input_neuron_freq = 10 # Hz
